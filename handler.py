...
def handler(job):
    ...
    try:
        ...
        for node_id, node_output in outputs.items():
            ...
            # Handle text outputs (for VLM captioning workflows)
            if "text" in node_output:
                text_outputs = node_output["text"]
                print(
                    f"worker-comfyui - Node {node_id} contains {len(text_outputs)} text output(s)"
                )
                for i, text_content in enumerate(text_outputs):
                    if text_content:
                        output_data.append(
                            {
                                "node_id": node_id,
                        ...
                        print(f"worker-comfyui - Captured text output from node {node_id}")
    ...
    # Separate image and text outputs
    image_outputs = [o for o in output_data if o.get("type") in ("base64", "s3_url")]
    text_outputs = [o for o in output_data if o.get("type") == "text"]

    if image_outputs:
        final_result["images"] = image_outputs

    if text_outputs:
        final_result["text"] = text_outputs
    ...
    output_summary = []
    if image_outputs:
        output_summary.append(f"{len(image_outputs)} image(s)")
    if text_outputs:
        output_summary.append(f"{len(text_outputs)} text output(s)")
    print(f"worker-comfyui - Job completed. Returning {', '.join(output_summary) or 'no outputs'}.")
    return final_result
